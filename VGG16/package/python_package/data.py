...
class data_for_train:
    def __init__(self,data_lists,mode,augument):
        self._mode = mode
        self._augument = augument
        self._all_files = data_lists
    def create_train_for_generator(self):
        images = []
        dataset_info = self._all_files.values
        # print("-----------------------")
        # print(self._mode)
        # print(self._augument)
        ...
        while 1:
            ...
            for start in range(0,len(dataset_info),config.batch_size):
                ...
                batch_labels = np.zeros((len(X_train_batch),config.nb_classes))
        ...
        while 1:
            ...
            for start in range(0,len(dataset_info),config.batch_size):
                ...
                for i in range(len(X_train_batch)):
                    ...
                    image = cv2.imread(X_train_batch[i][0])
                    image = cv2.resize(image,(config.image_size,config.image_size),interpolation=cv2.INTER_NEAREST)
                    # image = cv2.resize(image,(config.image_size,config.image_size),Image.LANCZOS).convert('L')
                    ...
                    if not self._mode == "test":
                        batch_labels[i][X_train_batch[i][1]] = 1
                        #print(np.array(batch_images).shape)
                return np.array(batch_images, np.float32), batch_labels
...
def get_files(root,mode):
    ...
    for line in lines:
        ...
        num_labels.append(int(curLine[0]))
